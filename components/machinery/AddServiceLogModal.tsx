      ...
      <div className="bg-[#111827] border border-white/[0.06] rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-[#F1F5F9]">Log Service</h2>
              <p className="text-xs text-[#64748B]">Record completed maintenance work</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-4">
            ...
            {selectedAsset && (
              <div className="flex gap-4 mt-1">
                {selectedAsset.hoursTotal != null && (
                  <p className="text-[10px] text-[#64748B]">Current hours: {selectedAsset.hoursTotal.toLocaleString()}</p>
                )}
                {selectedAsset.kmTotal != null && (
                  <p className="text-[10px] text-[#64748B]">Current km: {selectedAsset.kmTotal.toLocaleString()}</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wide">Date *</label>
          ...
          <div className={`grid gap-4 ${isPowerUnit ? "grid-cols-4" : "grid-cols-3"}`}>
            <div>
              <label className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wide">Cost ($)</label>
              <input type="number" step="0.01" value={form.cost} onChange={e => update("cost", e.target.value)} placeholder="0.00"
                className="w-full mt-1 bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-[#F1F5F9] focus:border-[#34D399]/40 focus:outline-none" />
            </div>
            <div>
              <label className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wide">Hours at Service</label>
              <input type="number" value={form.hoursAtService} onChange={e => update("hoursAtService", e.target.value)} placeholder="0"
                className="w-full mt-1 bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-[#F1F5F9] focus:border-[#34D399]/40 focus:outline-none" />
            </div>
            {isPowerUnit && (
              <div>
                <label className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wide">KM at Service</label>
                <input type="number" value={form.kmAtService} onChange={e => update("kmAtService", e.target.value)} placeholder="0"
                  className="w-full mt-1 bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-[#F1F5F9] focus:border-[#34D399]/40 focus:outline-none" />
              </div>
            )}
            <div>
              <label className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wide">Labor Hours</label>
          ...
          {/* Parts Used */}
          <div>
            <label className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wide">Parts Used</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wide">Vendor / Shop</label>
              <input type="text" value={form.vendor} onChange={e => update("vendor", e.target.value)} placeholder="e.g., Prairie Equipment"
                className="w-full mt-1 bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-[#F1F5F9] focus:border-[#34D399]/40 focus:outline-none" />
            </div>
            <div>
              <label className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wide">Performed By</label>
          ...
          {/* Attachments */}
          <div>
            <label className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wide mb-2 block">Attachments</label>
              ...
              <div className="space-y-2 mb-3">
                {attachments.map((att, i) => (
                  <div key={i} className="flex items-center gap-3 bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2">
                    ...
                    <button onClick={() => removeAttachment(i)} className="text-[#64748B] hover:text-[#EF4444] transition-colors">
                      <Trash2 size={12} />
                    </button>
                  </div>
                ))}
            ...
            <button onClick={() => fileRef.current?.click()} disabled={uploading}
              className="flex items-center gap-2 text-xs font-semibold text-[#94A3B8] border border-dashed border-white/[0.1] rounded-lg px-4 py-3 w-full hover:border-[#34D399]/30 hover:text-[#34D399] transition-colors disabled:opacity-50">
              {uploading ? (
            ...
            <p className="text-[10px] text-[#475569] mt-1">PDF, images, Word, Excel — max 10MB each</p>
          </div>
        </div>
