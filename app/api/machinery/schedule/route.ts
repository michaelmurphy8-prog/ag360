...
export async function POST(req: NextRequest) {
  ...
  const [schedule] = await sql`
    INSERT INTO "ServiceSchedule" (
      "orgId", "assetId", "serviceType", "intervalHours", "intervalDays",
      "dueAtHours", "dueAtDate", "priority", "notes"
    ) VALUES (
      ${tenantId}, ${assetId}, ${serviceType},
      ${intervalHours || null}, ${intervalDays || null},
      ${dueAtHours || null}, ${dueAtDate || null},
...
export async function DELETE(req: NextRequest) {
  ...
  await sql`DELETE FROM "ServiceSchedule" WHERE id = ${id} AND "orgId" = ${tenantId}`;
  return NextResponse.json({ success: true });
}