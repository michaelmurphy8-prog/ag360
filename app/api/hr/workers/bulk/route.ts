...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (let i = 0; i < workers.length; i++) {
      ...
      try {
        await sql`
          INSERT INTO workers (user_id, name, role, worker_type, status, phone, email, emergency_contact, emergency_phone, hourly_rate, daily_rate, start_date, end_date, notes)
          VALUES (${userId}, ${w.name.trim()}, ${w.role?.trim() || null}, ${mappedType}, ${status}, ${w.phone?.trim() || null}, ${w.email?.trim() || null}, ${w.emergency_contact?.trim() || null}, ${w.emergency_phone?.trim() || null}, ${hourlyRate}, ${dailyRate}, ${startDate}, ${endDate}, ${w.notes?.trim() || null})
        `;
        results.inserted++;
      } catch (err: any) {
