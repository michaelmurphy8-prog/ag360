...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [{ count }] = await sql`
      ...
      WHERE tenant_id = ${tenantId} AND crop_year = ${parseInt(cropYear)} AND is_void = false
    `;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const {
      entry_date, description, memo, source, crop_year, field_name, crop, lines,
      vendor, document_number, document_type, payment_status, payment_terms,
      due_date, ai_scanned, paid_date
    } = body;
    ...
    const [entry] = await sql`
      INSERT INTO journal_entries (
        tenant_id, entry_date, description, memo, source, crop_year, field_name, crop,
        vendor, document_number, document_type, payment_status, payment_terms,
        due_date, ai_scanned, paid_date
      ) VALUES (
        ${tenantId}, ${entry_date}, ${description}, ${memo || null},
        ${source || "manual"}, ${crop_year || 2025}, ${field_name || null}, ${crop || null},
        ${vendor || null}, ${document_number || null}, ${document_type || "manual"},
        ${payment_status || "paid"}, ${payment_terms || null},
        ${due_date || null}, ${ai_scanned || false}, ${paid_date || null}
      )
      RETURNING *
    `;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      await sql`
        INSERT INTO journal_lines (
          ...
          ${entry.id}, ${line.account_id}, ${line.description || null},
          ${parseFloat(line.debit) || 0}, ${parseFloat(line.credit) || 0},
          ${line.quantity ? parseFloat(line.quantity) : null}, ${line.unit || null},
          ${line.unit_price ? parseFloat(line.unit_price) : null},
          ${line.field_name || field_name || null}, ${line.crop || crop || null}, ${i}
        )
      `;
    }
