...
import {
  ...
  Briefcase, Calendar, Wheat, Plus, Pencil, Trash2, Handshake, Link,
  ...
  useEffect(() => {
    fetch('/api/connect360/requests')
      ...
      .then(data => {
        ...
        setConnectedIds(new Set<string>(reqs.map((r: any) => r.profile_id)))
        setConnections(reqs)
      })
      .catch(() => {})
  }, [])
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
        ...
        <button
          ...
          className="flex items-center gap-3 p-3 rounded-xl border transition-all text-left"
          style={{
            backgroundColor: showConnectionsPanel ? 'rgba(212,175,55,0.08)' : 'var(--ag-bg-card)',
            borderColor: showConnectionsPanel ? 'rgba(212,175,55,0.4)' : 'var(--ag-border)',
          }}>
          <Link size={16} style={{ color: 'var(--ag-accent)' }} />
          <div>
      ...
      {showConnectionsPanel && (
        <div className="fixed inset-0 z-40 flex justify-end">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setShowConnectionsPanel(false)} />
          <div className="relative w-full max-w-md h-full flex flex-col shadow-2xl overflow-hidden"
            style={{ backgroundColor: 'var(--ag-bg-primary)', borderLeft: '1px solid var(--ag-border)' }}>
            <div className="flex items-center justify-between p-5 border-b" style={{ borderColor: 'var(--ag-border)' }}>
              <div className="flex items-center gap-2">
                <Link size={16} style={{ color: 'var(--ag-accent)' }} />
                <h2 className="font-bold text-ag-primary">My Connections</h2>
              ...
              <button onClick={() => setShowConnectionsPanel(false)} className="text-ag-muted hover:text-ag-primary text-lg">✕</button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {connections.length === 0 ? (
                <div className="text-center py-12 text-ag-muted text-sm">No connections yet. Connect with a provider to see them here.</div>
              ) : (
                connections.map((c: any) => {
                  const cfg = TYPE_CONFIG[c.type] ?? TYPE_CONFIG.worker
                  ...
                  return (
                    <div key={c.id} className="p-4 rounded-xl border"
                      ...
                      <div className="flex items-start gap-3 mb-3">
                        <div className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0"
                          ...
                          {c.photo_url
                            ...
                            : <CIcon size={15} className={cfg.color} />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="font-semibold text-sm text-ag-primary">
                            {c.business_name || `${c.first_name} ${c.last_name}`}
                          </div>
                          <div className="text-xs text-ag-muted">{c.first_name} {c.last_name}</div>
                          <div className="text-[10px] text-ag-dim mt-0.5">
                            {[c.base_city, c.base_province].filter(Boolean).join(', ')}
                          </div>
                        </div>
                        <a href={`/connect360/profile/${c.profile_id}`}
                          className="text-[10px] text-ag-muted hover:text-ag-accent transition-colors">
                          View →
                        </a>
                      </div>
                      <div className="flex gap-2">
                        {c.phone && (
                          <a href={`tel:${c.phone}`}
                            className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border text-xs transition-all hover:border-[var(--ag-accent-border)]"
                            style={{ borderColor: 'var(--ag-border)', color: 'var(--ag-text-secondary)' }}>
